...
export default function SantanderVip() {
    ...
    return (
        <div
            ...
            } p-6 sm:p-10 ${eras[currentEra].extraStyles || ""}`}
        >
            <motion.div
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 1.2, ease: "easeOut" }}
                className="p-6 sm:p-12 text-center backdrop-blur-md rounded-3xl shadow-2xl border border-gray-500 relative w-full max-w-lg"
            >
                <motion.h1
                    initial={{ y: -50, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ delay: 0.5, duration: 1, ease: "easeOut" }}
                    className={`text-4xl sm:text-6xl font-extrabold uppercase tracking-widest drop-shadow-lg ${eras[currentEra].textColor}`}
                >
                    S∀NTANDER,
                </motion.h1>
                <motion.h2
                    ...
                    className={`text-5xl sm:text-7xl font-extrabold mt-4 drop-shadow-glow tracking-wide uppercase border-4 px-4 py-2 inline-block ${eras[currentEra].textColor}`}
                >
                    Me dê um VIP!
                </motion.h2>
                <motion.button
                    ...
                    className={`mt-6 sm:mt-8 px-6 sm:px-8 py-3 sm:py-4 text-lg sm:text-xl font-bold rounded-full shadow-xl hover:scale-110 transition-all uppercase tracking-widest ${eras[currentEra].buttonColor} text-white`}
                    onClick={switchEra}
                >
